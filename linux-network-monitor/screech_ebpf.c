...
// Conditionally include CO-RE headers based on kernel support
#if HAVE_BPF_CORE_SUPPORT
...
static __always_inline void get_process_path(struct connection_event *event) {
    ...
    if (bpf_probe_read_kernel(&exe_file, sizeof(exe_file), &mm->exe_file) != 0 || !exe_file) {
        event->filename[0] = '\0';
        return;
    }
    
    struct dentry *dentry;
    if (bpf_probe_read_kernel(&dentry, sizeof(dentry), &exe_file->f_path.dentry) != 0 || !dentry) {
        event->filename[0] = '\0';
        return;
    }
    
    struct qstr d_name;
    if (bpf_probe_read_kernel(&d_name, sizeof(d_name), &dentry->d_name) != 0) {
        event->filename[0] = '\0';
        return;
    }
...
int trace_tcp_connect(struct pt_regs *ctx) {
    ...
    // Extract destination from sockaddr - with error checking
    __u16 sa_family;
    if (bpf_probe_read_kernel(&sa_family, sizeof(sa_family), &uaddr->sa_family) == 0) {
        if (sa_family == AF_INET) {
            struct sockaddr_in addr_in;
            if (bpf_probe_read_kernel(&addr_in, sizeof(addr_in), uaddr) == 0) {
...
#else
    if (bpf_probe_read_kernel(&addr, sizeof(addr), &msg->msg_name) != 0) {
        addr = NULL;
    }
    if (bpf_probe_read_kernel(&addr_len, sizeof(addr_len), &msg->msg_namelen) != 0) {
        addr_len = 0;
    }
#endif
