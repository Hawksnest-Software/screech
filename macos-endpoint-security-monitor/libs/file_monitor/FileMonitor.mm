...
public:
    ...
    bool initialize() {
        ...
        if (result != ES_NEW_CLIENT_RESULT_SUCCESS) {
            DEBUG_LOG_ERROR("Failed to create File Endpoint Security client: %d", result);
            return false;
        }
        ...
        if (subscribe_result != ES_RETURN_SUCCESS) {
            DEBUG_LOG_ERROR("Failed to subscribe to file events: %d", subscribe_result);
            es_delete_client(esClient);
            esClient = nullptr;
            return false;
        }
        
        DEBUG_LOG_FILESYSTEM("File monitoring initialized successfully");
        return true;
#else
        DEBUG_LOG_ERROR("File monitoring only supported on macOS");
        return false;
#endif
    }
    ...
    bool start() {
        ...
        isActive = true;
        DEBUG_LOG_FILESYSTEM("File monitoring started");
        return true;
    }
...
#endif
        DEBUG_LOG_FILESYSTEM("File monitoring stopped");
    }
...
private:
    ...
    void logFileEvent(const FileEvent& event) {
        // Debug logging
        const char* eventTypeStr = "";
        switch (event.type) {
            case FileEvent::OPEN: eventTypeStr = "FILE_OPEN"; break;
            case FileEvent::CLOSE: eventTypeStr = "FILE_CLOSE"; break;
            case FileEvent::WRITE: eventTypeStr = "FILE_WRITE"; break;
            case FileEvent::CREATE: eventTypeStr = "FILE_CREATE"; break;
            case FileEvent::DELETE: eventTypeStr = "FILE_DELETE"; break;
            case FileEvent::RENAME: eventTypeStr = "FILE_RENAME"; break;
        }
        
        DEBUG_LOG_FILESYSTEM("%s: %s (PID: %d, Process: %s)", 
                           eventTypeStr, event.filePath.c_str(), event.process.pid, event.process.name.c_str());
        
        // Use EndpointSecurityCore logging
        EndpointSecurityCore::EndpointSecurityCoreEngine::logFileSystemEvent(
            eventTypeStr, event.filePath, event.details, event.process, event.timestamp);
    }
