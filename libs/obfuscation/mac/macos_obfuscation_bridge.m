...
long obfuscated_syscall(int syscall_number, void *args, int arg_count) {
    ...
    if (arg_count < 0 || arg_count > 8) {
        DEBUG_LOG_WARNING("Invalid syscall argument count: %d", arg_count);
        return -1;
    }
    ...
    // Add obfuscation based on argument count
    volatile uint64_t noise = (uint64_t)arg_count * 0x1337;
    for (int i = 0; i < arg_count; i++) {
        noise ^= (noise << 1) + i;
    }
    (void)noise; // Suppress unused warning
    
    // Perform the actual syscall with inline assembly
    long result = 0;
    
    DEBUG_LOG_DEBUG("Executing obfuscated syscall %d with %d arguments", 
...
#ifdef ENABLE_RUNTIME_INTROSPECTION
