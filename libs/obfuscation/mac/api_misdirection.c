...
void init_api_misdirection(void) {
    ...
    DEBUG_LOG_DEBUG("API Misdirection Initialized");
}

void activate_misdirection(bool enable) {
    misdirection_registry.misdirection_active = enable;
    DEBUG_LOG_DEBUG("Misdirection %s", enable ? "activated" : "deactivated");
}
...
void enable_selective_misdirection(const char* target_functions[], uint32_t count) {
    if (!target_functions || count == 0) {
        DEBUG_LOG_WARNING("Invalid parameters for selective misdirection");
        return;
    }
    
    DEBUG_LOG_INFO("Enabling selective misdirection for %u functions", count);
    
    // Store target functions for selective obfuscation
    for (uint32_t i = 0; i < count && i < MAX_MISDIRECTION_TARGETS; i++) {
        if (target_functions[i]) {
            // Hash the function name for fast lookup
            uint32_t hash = 0;
            for (const char* p = target_functions[i]; *p; p++) {
                hash = hash * 31 + *p;
            }
            
            // Store in registry for selective targeting
            if (misdirection_registry.fake_api_count < MAX_FAKE_APIS) {
                misdirection_registry.fake_apis[misdirection_registry.fake_api_count].api_hash = hash;
                misdirection_registry.fake_apis[misdirection_registry.fake_api_count].call_frequency = 1;
                misdirection_registry.fake_api_count++;
                
                DEBUG_LOG_DEBUG("Added function '%s' to misdirection targets (hash: 0x%x)", 
                               target_functions[i], hash);
            }
        }
    }
    
    // Enable selective mode
    misdirection_registry.selective_mode = true;
    DEBUG_LOG_INFO("Selective misdirection enabled for %u target functions", 
                   misdirection_registry.fake_api_count);
}
